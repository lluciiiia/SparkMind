...
export const RecentLearnings = () => {
  return (
    <section className={`flex flex-col gap-4`}>
      ...
      <article className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2`}>
        {Array.from({ length: 5 }, (_, index) => (
          <RecentLearningsItem key={index} index={index} />
        ))}
      </article>
    </section>
  );
...
const RecentLearningsItem = ({ index }: { index: number | string }) => {
  return (
    <Card className={`w-full h-[150px] sm:h-[200px] md:h-[250px] flex items-center justify-center`}>
      <CardHeader>
        <CardTitle>{`Learning ${index}`}</CardTitle>
      </CardHeader>
    </Card>
  );
};
