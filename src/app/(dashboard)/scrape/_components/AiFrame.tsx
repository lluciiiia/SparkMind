...
const AiFrame: React.FC<{ topic: string; websiteData: string; uuid: string; isLoading: boolean }> = memo(({ topic, websiteData, uuid, isLoading: parentIsLoading }) => {
  ...
  const model = useMemo(() => googleGenerativeAI.getGenerativeModel({
  ...
  const generateContent = useCallback(async (topic: string, websiteData: string) => {
  ...
  const debouncedGenerateContent = useMemo(() => debounce(generateContent, 1000), [generateContent]);
